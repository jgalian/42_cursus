	...
	class map
	{
		...
		private:
			...
			void _add_node(node_ptr to_add)
			{
				...
				while (*curr && *curr != endnode)					// La primera vez no entra pero luego entra siempre
				{
				...
				if (*curr == NULL)									// Entra cuando llega a un sitio donde insertar el nuevo elemento, y no es el endnode
				{
