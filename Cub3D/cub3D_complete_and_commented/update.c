...
void	update(t_all *a)
{
	...
	a->play.rotation_angle += a->play.turn_dir * a->play.turn_speed;		// Compute the new player rotation angle
	move_step = a->play.walk_dir * a->play.walk_speed;						// Calculate the space player is going to move forward or backward
	if (move_step > a->ray[a->mlx.win_width / 2].distance)
		move_step = a->ray[a->mlx.win_width / 2].distance - 5;
	new_player_x = a->play.x + cos(a->play.rotation_angle +					// Calculate new player's cooordinates based on new move_step and angle
	...
	update_sprites(a);														// Call function to update sprites data
	sort_sprites(a);														// Call a function to sort sprites based on distance to player
}
...
int		is_new_position_ok(t_all *a, float new_map_x, float new_map_y, int map[])
{
	if (new_map_x > map[0] && new_map_y < map[1])
	{
