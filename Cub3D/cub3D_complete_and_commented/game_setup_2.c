...
void    get_wall_textures(t_all *a)											// Get data from texture files...
{																			// ... and save the pointer to the array with the pixels
	int		i;
...
void	get_sprites_textures(t_all *a)										// Get data from texture files...
{																			// ... and save the pointer to the array with the pixels
	int		i;

	a->spr_text[0].img = mlx_xpm_file_to_image(a->mlx.mlx,
		a->param.text_spr, &a->spr_text[0].width, &a->spr_text[0].height);
	i = 0;
    while(i < 1)
    {
...
void	count_sprites(t_all *a)												// Count numbers of sprites in the map
{
	int		i;
	int		j;
...
void	get_sprite_data(t_all *a)											// Save the position (based on resolution) of each sprite
{
	int		i;
	int		j;
	int		k;
